...
def compute_response(sentence):
    ...
    if greeting_np[np.argmax(greeting_np)] > 0.5:
        ...
        return "ขอโทษ ฉันไม่เข้าใจคำถามของคุณกรุณากรอกใหม่อีกครั้ง"
...
def save_chat_history(user_id, user_msg, bot_reply):
    query = '''
    MERGE (u:User {uid: $user_id})  // สร้างหรือค้นหาผู้ใช้จาก userId
    CREATE (m:Message {text: $user_msg, timestamp: $timestamp})  // สร้าง node สำหรับข้อความที่ผู้ใช้ส่ง
    CREATE (r:Reply {text: $bot_reply, timestamp: $timestamp})  // สร้าง node สำหรับข้อความตอบกลับจากบอท
    MERGE (u)-[:SENT]->(m)  // สร้างความสัมพันธ์ระหว่างผู้ใช้กับข้อความที่ส่ง
    MERGE (m)-[:HAS_REPLY]->(r)  // สร้างความสัมพันธ์ระหว่างข้อความและการตอบกลับ
    '''
    parameters = {
        "user_id": user_id,  
        "user_msg": user_msg,  
...
def ask_style(reply_token, selected_style=None):
...
def ask_color(reply_token, selected_color=None):
...
def ask_orderby(reply_token, selected_order=None):
...
def ask_reset(reply_token):
    quick_reply = QuickReply(items=[
        QuickReplyButton(action=MessageAction(label="รองเท้าชาย", text="รองเท้าชาย")),
        QuickReplyButton(action=MessageAction(label="รองเท้าผู้หญิง", text="รองเท้าผู้หญิง")),
        QuickReplyButton(action=MessageAction(label="รองเท้าเด็ก", text="รองเท้าเด็ก")),
        QuickReplyButton(action=MessageAction(label="ของตกแต่ง", text="ของตกแต่ง")),
        QuickReplyButton(action=MessageAction(label="รองเท้าลดราคา", text="รองเท้าลดราคา"))
    ])

    line_bot_api.reply_message(
        reply_token,
        TextSendMessage(text="กรุณาเลือกประเภทสินค้าชิ้นใหม่ : ", quick_reply=quick_reply)
    )

quick_reply_neo4j = QuickReply(items=[
    QuickReplyButton(action=MessageAction(label="รองเท้าชาย", text="รองเท้าชาย")),
    QuickReplyButton(action=MessageAction(label="รองเท้าผู้หญิง", text="รองเท้าผู้หญิง")),
    QuickReplyButton(action=MessageAction(label="รองเท้าเด็ก", text="รองเท้าเด็ก")),
    QuickReplyButton(action=MessageAction(label="ของตกแต่ง", text="ของตกแต่ง")),
...
def linebot():
    ...
    try:
        ...
        selected_color = selected_color if 'selected_color' in globals() else None
        selected_style = selected_style if 'selected_style' in globals() else None
         # URL mappings
        url_map = {
            "รองเท้าชาย": "https://crocs.co.th/collections/men",
            "รองเท้าผู้หญิง": "https://crocs.co.th/collections/women",
        }
        url_map_promo = {
            "ของตกแต่ง": "https://crocs.co.th/collections/jibbitz-charms",
            "รองเท้าเด็ก": "https://crocs.co.th/collections/kids",
            "รองเท้าลดราคา": "https://crocs.co.th/collections/sale"    
        }
        ...
        # Handle style selection and then ask for color
            ...
            final_url = (category_url)+(selected_style)  # No need to append style or color filters
            products = scrape_crocs(final_url)
            bot_reply = "นี่คือผลการค้นหาครับ"
            send_flex_message(reply_token, products,bot_reply)
            save_chat_history(user_id, msg, bot_reply)
        ...
        # Handle color selection and generate the final URL
            ...
            elif selected_style and not selected_color:
                final_url = f"{category_url}?{selected_style}&{selected_order}"
            else :
                final_url = f"{category_url}?{selected_order}"
            products = scrape_crocs(final_url)
            print(final_url)
            print(category_url)
         ...
         # กรณีที่เป็นข้อความปกติที่ต้องการค้นหาการตอบกลับใน Neo4j
            ...
            line_bot_api.reply_message(reply_token, TextSendMessage(text=bot_reply, quick_reply=quick_reply_neo4j))
